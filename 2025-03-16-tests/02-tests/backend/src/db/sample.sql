...
LEFT OUTER JOIN `comments` AS `posts->comments`
ON
    `posts`.`id` = `posts->comments`.`post_id`
LEFT OUTER JOIN `users` AS `posts->comments->user`
ON
    `posts->comments`.`user_id` = `posts->comments->user`.`id`
LEFT OUTER JOIN `users` AS `posts->user`
ON
    `posts`.`user_id` = `posts->user`.`id`
WHERE
